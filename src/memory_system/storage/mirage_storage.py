...
class MirageStorage:
    """Storage backend backed by a mirage Workspace (DiskResource).

    Files are written inside the DiskResource root so they appear
    under the mirage VFS mount point. The workspace is kept alive
    ...
    def __init__(self, base_path: str):
        ...
        try:
            from mirage import Workspace, DiskResource, WorkspaceRunner, MountMode
            self._ws = Workspace(
                {"/data": DiskResource(str(self._base))},
                mode=MountMode.WRITE,
            )
            self._runner = WorkspaceRunner(self._ws)
        except ImportError:
            logger.warning(
                "mirage-ai not installed; MirageStorage will use direct disk I/O. "
                "Install with: pip install mirage-ai"
            )
    ...
    def _classify(path_or_url: str) -> str:
        return "assets" if MirageStorage._is_media(path_or_url) else "raw"
    ...
    async def save_file(self, user_id: str, file_url: str) -> str | None:
        ...
        try:
            ...
            logger.info(f"[mirage] Saved file: {file_url} -> {dest_path}")
            return f"{category}/{dest_name}"
        except Exception as e:
            logger.error(f"[mirage] Failed to save file {file_url}: {e}")
            return None

    async def write_journal(self, user_id: str, entries: list[dict]):
        ...
        try:
            ...
            logger.info(f"[mirage] Journal written: {journal_path} ({len(entries)} entries)")
        except Exception as e:
            logger.error(f"[mirage] Failed to write journal: {e}")
